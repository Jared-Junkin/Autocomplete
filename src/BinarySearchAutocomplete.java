...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public BinarySearchAutocomplete(String[] terms, double[] weights) {
		...
		//comment this if statement out if you get unexpected errors
	...
	public List<Term> topMatches(String prefix, int k) {
		...
		//a comparator that is comparing terms by their weight
		//the issue with this, at first the ten terms weren't sorted at all, and I also wasn't adding any new terms to the priority queue
		for(int x = first; x <= last; x++) {
			if (pq.size() < k) {
				pq.add(myTerms[x]);
			}
			else{
				if(myTerms[x].getWeight() > pq.peek().getWeight()){
					pq.remove();
					pq.add(myTerms[x]);
				}
			}
		}
		LinkedList<Term> front = new LinkedList<>();
		while(pq.size() > 0){
			front.addFirst(pq.remove());
		}
		return front;
